...
const IncomeManager: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setIncomes(transResp.data);
      setCategories(catResp.data.filter((c: any) => c.type === 'income'));
    } catch (err) {
      console.error(err);
    }
  };
  ...
  return (
    <Box sx={{ animation: 'fadeIn 0.5s ease-out' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 6, alignItems: 'flex-start' }}>
        <Box>
          <Typography variant="h4" fontWeight="900" sx={{ letterSpacing: '-1.5px', mb: 1 }}>Income Manager</Typography>
          <Typography variant="body1" color="text.secondary" fontWeight="500">Track and manage your revenue streams.</Typography>
        </Box>
        <Button 
          variant="contained" 
          startIcon={<Add />} 
      ...
      <TableContainer component={Paper} sx={{ borderRadius: '24px', overflow: 'hidden', border: '1px solid rgba(255, 255, 255, 0.05)' }}>
        <Table>
          ...
          <TableBody>
            {incomes.map((row: any, idx: number) => (
              <TableRow 
                key={row.id} 
                hover
                sx={{ 
      ...
      >
        <DialogTitle sx={{ fontWeight: 900, letterSpacing: '-0.5px', pt: 3, px: 3 }}>Add New Income</DialogTitle>
        ...
        <DialogActions sx={{ p: 3, gap: 1 }}>
          ...
          <Button onClick={handleSubmit} variant="contained" sx={{ fontWeight: 800, textTransform: 'none', px: 4, borderRadius: '10px' }}>Save Income</Button>
        </DialogActions>
      </Dialog>
    </Box>
  );
